...
CORS_ALLOWED_ORIGINS = [
    "http://localhost:5173",
    "http://localhost:8000",
    "https://invoiceflow-vnvs.onrender.com",
]
...
# Code-defined periodic tasks (default Celery beat scheduler reads this).
# Use `celery -A config beat` without `--scheduler django_celery_beat...` so these run.
# django-celery-beat remains installed for optional DB schedules / admin if you switch scheduler later.
CELERY_BEAT_SCHEDULE = {
    'send-due-invoice-reminders-daily': {
        'task': 'apps.notifications.tasks.send_due_invoice_reminder',
        'schedule': crontab(hour=8, minute=0),
    },
}

# Redis Cache Configuration
# Use django-redis backend — CLIENT_CLASS is only valid here, not on Django's built-in RedisCache.
CACHES = {
    'default': {
...
# Email — outbound via Resend HTTPS API (no SMTP). See utils/email_backends/resend_backend.py
RESEND_API_KEY = os.environ.get('RESEND_API_KEY', '')
EMAIL_BACKEND = os.environ.get(
    'EMAIL_BACKEND',
    'utils.email_backends.resend_backend.ResendEmailBackend',
)
# Default From when messages omit from_email. Without a verified domain, use Resend's test sender.
DEFAULT_FROM_EMAIL = os.environ.get('DEFAULT_FROM_EMAIL', 'noreply@mail.shaidur.com.bd')
...
LOGGING = {
    ...
    'root': {
        'handlers': ['console'],           # Only console on Render
        'level': 'INFO',
    },

    'loggers': {
        'django': {
            'handlers': ['console'],       # Removed 'file' for Render
            'level': 'INFO',
            'propagate': False,
        },
...
SPECTACULAR_SETTINGS = {
    ...
    'DESCRIPTION': 'SaaS Invoice & Billing Platform API Documentation',
    'VERSION': '1.0.0',
